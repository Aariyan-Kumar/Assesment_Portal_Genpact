...
public class UpdateTestInfo extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			String query2 = "UPDATE testdata SET subjectName = ?, numOfQuestion = ?, passMarks = ? WHERE id = ?";
			PreparedStatement ps2 = conn.prepareStatement(query2);
			ps2.setString(1, testName);
			ps2.setInt(2, oldNumOfQuestion + numOfQuestions);
			ps2.setInt(3, passMarks);
			ps2.setInt(4, testId);
			ps2.executeUpdate();
			response.sendRedirect("addQuestion.jsp?testName="+ URLEncoder.encode(testName, "UTF-8") + "&numOfQuestion=" + numOfQuestions);
			conn.close();
